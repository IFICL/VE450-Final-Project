...
class RAM(object):
    ...
    def build_model(self):
        with tf.name_scope('inputs'):
            self.sentences = tf.placeholder(tf.int32, [None, self.max_sentence_len], name="sentences")
            self.aspects = tf.placeholder(tf.int32, [None, self.max_aspect_len], name="aspects")
            self.sentence_lens = tf.placeholder(tf.int32, None, name="sentence_lens")
            self.sentence_locs = tf.placeholder(tf.float32, [None, self.max_sentence_len], name="sentence_locs")
        ...
        with tf.name_scope('predict'):
            self.predict_label = tf.argmax(self.predict, 1, name="predict_label")
