...
def get_data_info_csv(train_fname, test_fname, save_fname, pre_processed):
    word2id, max_sentence_len, max_aspect_len = {}, 0, 1
    word2id['<pad>'] = 0
    if pre_processed:
        if not os.path.isfile(save_fname):
            raise IOError(ENOENT, 'Not a file', save_fname)
        ...
        for sentence in comments:
            if sentence:
                sptoks = jieba.cut(sentence[0], cut_all=False)
                ...
                if len(sptoks.text.strip()) != 0:
                    ...
                    for asp_terms in sentence.iter('aspectTerms'):
                        for asp_term in asp_terms.findall('aspectTerm'):
                            ...
                            f.write("%s\n" % sentence_lens[-1])
                            loc_info = [0] #get_loc_info(sptoks, int(asp_term.get('from')), int(asp_term.get('to')))
...
def read_data_csv(fname, word2id, max_sentence_len, max_aspect_len, save_fname, pre_processed):
    sentences, aspects, sentence_lens, sentence_locs, labels = [], [], [], [], []
    if pre_processed:
        if not os.path.isfile(save_fname):
            raise IOError(ENOENT, 'Not a file', save_fname)
                ...
                if sentence[0]:
                    ...
                    for i in range(1, len(sentence), 2):
                        ...
                        if sentence[i] in word2id:
                            if sentence[i] not in sptoks:
                                continue
                            loc_index = sptoks.index(sentence[i])
                            t_ids.append(word2id[sentence[i]])
                        ...
                        sentence_lens.append(len(sptoks))
                        f.write("%s\n" % sentence_lens[-1])
                        sentence_locs.append([0]*loc_index + [1] + [0] * (max_sentence_len-1-loc_index))
                        f.write("%s\n" % sentence_locs[-1])
                        if sentence[i+1] == 'negative':
                            labels.append([1, 0, 0])
                        elif sentence[i+1] == 'neutral':
                            labels.append([0, 1, 0])
                        elif sentence[i+1] == "positive":
                            labels.append([0, 0, 1])
                        else:
                            print(sentence)
                        f.write("%s\n" % labels[-1])
